...
<?php
		...
		$query="select c.nombre_concierto, c.fecha_concierto, c.estado_concierto, s.estado_subasta from Concierto c, Subasta s
		where s.codigo_concierto=c.codigo_concierto and s.codigo_usuario=".$_SESSION['id_user'].";";
		$resultado=mysqli_query($con, $query);
		$rows=mysqli_num_rows($resultado);
		if($rows==0){
		echo "No te has inscrito a ningun concierto.<br/>";
		}
		else{
		echo "<table border=1>
		<tr><td>Nombre concierto</td><td>Fecha concierto</td><td>Estado del concierto</td><td>Estado candidatura</td></tr>";
