...
<?php
	...
	or die("No se ha podido conectar con la base de datos. Inténtalo más tarde.");
			...
			$user=$_POST['nombre_local'];
			$passwd_local=$_POST['contrasenya_local'];
			$correo_local=$_POST['correo_local'];
			$tlf_local=$_POST['telefono_local'];
			$direc_local=$_POST['direccion_local'];
			$genero_local=$_POST['genero_local'];
			...
			$query4="select * from Usuario where correo_usuario='$correo_local'";
			...
			//cuando tanto el correo como el nombre de usuario no estan repetidos
				...
				$pass_encriptado = password_hash($passwd_local, PASSWORD_BCRYPT, $options);
				$query5="insert into Usuario(nombre_usuario, contrasenya, correo_usuario, telefono_usuario, direccion_usuario, codigo_ciudad, codigo_genero, tipo) 
				values ('$user', '$pass_encriptado', '$correo_local', '$tlf_local', '$direc_local', '$ciudad_local', '$genero_local', 1)";
